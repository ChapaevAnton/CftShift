...
object SingleOperators {
    ...
    object Task3 {
        ...
        fun solve(provider: DataProvider): Single<Int> =
            Single.defer { Single.just(provider.get()) }
    ...
    object Task5 {
        ...
        fun solve(name: String, api: UserApi): Single<User> =
            Single.just(api).flatMap { itApi -> itApi.create(name).flatMap { id -> itApi.get(id) } }
    ...
    object Task6 {
        ...
        fun solve(userApi: UserApi): Single<User> =
            Singles.zip(userApi.getId(), userApi.getName(), userApi.getAge())
