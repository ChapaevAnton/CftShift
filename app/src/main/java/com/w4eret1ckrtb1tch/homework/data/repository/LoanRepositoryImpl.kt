...
class LoanRepositoryImpl @Inject constructor(
    ...
    private val loanMapper: LoanResponseMapper,
    private val conditionsMapper: ConditionsResponseMapper
) : LoanRepository {
    ...
    override fun getConditions(authToken: String): Single<LoanConditions> {
        return api.getConditions(authToken)
            .subscribeOn(Schedulers.io())
            .observeOn(Schedulers.computation())
            .map { conditionsMapper.mapResponse(it) }
    }
}